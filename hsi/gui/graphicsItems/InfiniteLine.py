    ...
    **Signals:**
    ...
    def __init__(self, pos=None, angle=90, pen=None, movable=False, bounds=None,
        ...
        **Arguments:**
        ...
        self.p = [0, 0]
        self.setAngle(angle)
        self.orthogLine = None  # orthogonal line for cross hair cursor
    ...
    def connect(self, line):
        check_class(line, InfiniteLine, allowNone=True)

        if (self.angle == 0 and line.angle == 90) or (self.angle == 90 and line.angle == 0):
            self.orthogLine = line
            logger.debug("Connect orthogonal line: {}".format(line))
        else:
    ...
    def isOrthogonalLineHovered(self, ev):
        ...
        lineLength = self._endPoints[1] - self._endPoints[0]

        if self.angle == 0:
            x = self.orthogLine.getXPos()
            logger.debug("Relative distance from orthogonal line: %f (%fpx)" %
                         (abs(p.x() - x), abs(p.x() - x) / lineLength))
            if abs(p.x() - x) / lineLength < .1:
                return True
            else:
                return False

        elif self.angle == 90:
            y = self.orthogLine.getYPos()
            logger.debug("Distance from orthogonal line: %f (%fpx)" %
                         (abs(p.y() - y), abs(p.y() - y) / lineLength))
            if abs(p.y() - y) / lineLength < .1:
                return True
            else:
                return False
    ...
    def mouseDragEvent(self, ev):
        ...
        if self.movable and ev.button() == QtCore.Qt.LeftButton :
            ...
            if not self.orthogLine is None: # do not drag if orthogonal line not hovered
                if not self.isOrthogonalLineHovered(ev):
                    return
            ...
            if not self.orthogLine is None:
                self.orthogLine.setPos(self.mapToParent(ev.pos()))
