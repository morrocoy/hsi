...
from hsi.misc import readExcelTbl
...
def plotComponents():
    compound = HSTissueCompound(skintype='dermis')
    compound.evaluate()
    ...
    for key, component in components.items():
        x = component.wavelen
        y = component.absorption
        ax.plot(x, y, label='%s' % key.upper(),
    ...
    filePath = os.path.join(pict_path, "tissue_components")
...
def testComponents():
    ...
    refparam = {}
    filePath = os.path.join(data_path, "Phantom builder parameters 2.xlsx")
    metadata = {
