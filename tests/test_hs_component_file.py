...
def main():
    ...
    formats = [HSAbsorption, HSAbsorption, HSIntensity, HSAbsorption,
               HSAbsorption]
    ...
    for i, name in enumerate(names):
        vectors[name] = HSComponent(
            ...
            hsformat=formats[i], weight=weights[i], bounds=bounds[i])
    ...
    fpath5 = os.path.join(
        data_path, "..", "hsi", "data", "hsvectorfile_test3.txt")
    ...
    with HSComponentFile(fpath3) as file:
        ...
        file.buffer(spec4, label="spec4", hsformat=HSAbsorption)
    ...
    rvectors['mel'].set_format(HSAbsorption)
    for key, vec in rvectors.items():
        print(key, vec.label, vec.weight, vec.bounds, vec.hsformat.key,
              vec.xIntpData[0], vec.yIntpData[0])
