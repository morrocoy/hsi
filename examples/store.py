...
def create_dataset(file_name, h5_path, descr=None):
    ...
    with HSStore.open(
            file_path, mode="w", path=h5_path, descr=descr) as dataset:
...
def task(args):
    ...
    hsimage = HSImage(spectra=hsidata["spectra"], wavelen=hsidata["wavelen"],
                      hsformat=hsformat)
    # image = hsimage.as_rgb()

    analysis = HSOpenTivita(hsformat=HSAbsorption)  # open source algorithms
    # analysis = HSTivita(hsformat=HSIntensity)  # true algorithms
...
def postprocess_dataset(file_name, mask_config=None):
...
def main():
    ...
    create_dataset(file_path, h5_path="/records", descr=__doc__)
    process_dataset(file_path)
    postprocess_dataset(file_path)
